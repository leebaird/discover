...
f_install_discover_robots_handler(){
    local apps_dir="$USER_HOME/.local/share/applications"
    local desktop="$apps_dir/discover-robots.desktop"
    local mimeapps="$USER_HOME/.config/mimeapps.list"
    local opener="$DISCOVER_ROOT/misc/open-robots-tabs.sh"
...
Version=1.0
Type=Application
Name=Discover Robots Tabs
Comment=Open each robots.txt Disallow URL in Firefox (Discover)
Exec=$opener %u
Terminal=false
Categories=Network;Security;
MimeType=x-scheme-handler/discover-robots;
NoDisplay=true
EOF
    ...
    if [ -f "$mimeapps" ]; then
        if grep -q 'x-scheme-handler/discover-robots=' "$mimeapps" 2>/dev/null; then
            sed -i 's|x-scheme-handler/discover-robots=.*|x-scheme-handler/discover-robots=discover-robots.desktop|' "$mimeapps"
        elif grep -q '^\[Default Applications\]' "$mimeapps"; then
            sed -i '/^\[Default Applications\]/a x-scheme-handler/discover-robots=discover-robots.desktop' "$mimeapps"
        else
    ...
    if [ -n "$SUDO_USER" ]; then
        chown "$SUDO_USER:" "$mimeapps" 2>/dev/null || true
        sudo -u "$SUDO_USER" xdg-mime default discover-robots.desktop x-scheme-handler/discover-robots >/dev/null 2>&1 || true
        sudo -u "$SUDO_USER" update-desktop-database "$apps_dir" >/dev/null 2>&1 || true
    else
