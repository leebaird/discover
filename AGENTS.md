...
- **Google Sheet:** per-engagement spreadsheet URL in `$REPORT_ROOT/tools/op-notes-url` (chmod 600). Empty URL disables sync. Not stored in `~/.discover/api-keys` (machine-global). Do **not** read a report-root `.env`. OAuth `client_secret.json` + `google-token.json` stay in `~/.discover/` (chmod 600). **Authorize** in this panel (browser OAuth); host-scan append never opens a browser. Client/defender Export omit `tools/op-notes-url` (and a stray report-root `.env` if present). Operator Export may keep the URL file. Host-scan: `misc/op_notes.py append` in the background (40s cap); failures must not abort the scan. Columns A–E: date, operator, IP, target, command. **nmap** waits until the scan finishes and writes the PORT table in column F (Result; Courier New). No open ports → `No open ports.` in the default font. Redact `--api-token` in logged commands. Bust `inc-audit-config.js?v=` / `modern.css` on Audit after UI changes.
