...
- **Software for expand:** `?software=` query wins; else fingerprint the row (Technologies tokens with version when present, title words so `Welcome to nginx!` matches nginx, web server, hostname labels — not only the first, so `uatapi.iis.cgi.com` matches IIS). Priority products include CMS, Kibana, Grafana, Elasticsearch, Jenkins, Kafka UI / Kafbat UI (hostname `kafka-ui` / title Kafbat), GitLab / Gitea / Gogs, Oracle (hostname `oci-adb-control` / realm OCI ADB Control; not the bare `oci` cluster label), Tomcat, IIS, nginx, Apache, ASP.NET, PHP, Node.js, Java (not JavaScript).
