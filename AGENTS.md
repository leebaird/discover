...
## Audit log table (hidden rows)

- Still written to `tools/audit/log.txt`. The Audit log table (and defender CSV) omit: **Opened report in Discover**, **Exported …**, nuclei pass-2, **Imported operator package** (imported lines stay), **Finished robots** (`Finished robots in 2 sec.` / `Finished robots on …`). **Started robots** stays (command + TXT/WEB). Metrics still count Finished robots.
...
- Modal hub with **four choice rows**: **Operator name**, **Time zone**, **APIs**, **Google Sheet**.
- Backend: `recon/discover-config.py` via statusd `GET /config`, `POST /config/api-keys`, `POST /config/operator-name`, `POST /config/timezone`, `POST /config/op-notes`, `POST /config/op-notes-authorize`.
...
- **Google Sheet:** per-engagement spreadsheet URL in `$REPORT_ROOT/tools/op-notes-url` (chmod 600). Empty URL disables sync. Not stored in `~/.discover/api-keys` (machine-global). Do **not** read a report-root `.env`. OAuth `client_secret.json` + `google-token.json` stay in `~/.discover/` (chmod 600). **Authorize** in this panel (browser OAuth); host-scan append never opens a browser. Client/defender Export omit `tools/op-notes-url` (and a stray report-root `.env` if present). Operator Export may keep the URL file. Host-scan: `uv run misc/op_notes.py append` in the background (40s cap; Discover install provides `uv`). Failures print on the scan (stderr) and must not abort the tool. Columns A–E: date, operator, IP, target, command. **nmap** waits until the scan finishes and writes the PORT table in column F (Result; Courier New, using the append `updatedRange` so concurrent scans do not format the wrong cell). No open ports → `No open ports.` in the default font. Redact `--api-token` in logged commands. Bust `inc-audit-config.js?v=` / `modern.css` on Audit after UI changes.
...
- Applies only to tools launched from the Subdomains expand panel: **nuclei**, **robots**, **nikto**, **ffuf**, **feroxbuster**, plus **droopescan** / **wpscan** when shown. **nmap** does not use this HTTP gate.
...
- **Nikto HTM** and **robots WEB** are omitted when `skip_reason=host_unreachable` (no report / no Disallow list).
...
- **robots** / **nikto** / **feroxbuster** / **ffuf** always on expand when the UI is shown. **nmap** after robots when that IP has Shodan ports, **or** the row URL is http/https (then include **80** or **443** even if Shodan is empty / `not_found`). Split TCP/UDP from Shodan host JSON `data[].transport`; if transport is missing, ports on the SCANNING `UDP=` list (`scan/nmap.sh`) are UDP, else TCP. **Port 53 is always both TCP and UDP** when it is in the Shodan set. TCP-only: `nmap -Pn -n --open -sTV -p <ports> <host>` (no sudo). Any UDP: `sudo nmap --privileged -Pn -n --open -sTV -sUV -p T:<tcp>,U:<udp> <host>` (`sudo -v` in the host-scan terminal if no cached ticket; abort if sudo fails — do not fall back to TCP-only). Already root skips sudo. No HTTP pre-check. If nmap raw logs `sendto Operation not permitted` (typical on VPN/TUN), TXT warns and rewrites UDP rows (`tcpwrapped` is not a fingerprint; VERSION `UDP probe blocked locally`). TCP rows stay. **web** (Firefox tabs) when SERVICE is http/https (`ssl/http` and `ssl/unknown` → https). Order: **robots** → nmap (if Shodan ports or http/https) → nuclei (if product) → CMS tools → nikto → feroxbuster → ffuf.
- **robots:** expand box label is **robots.txt** (not uppercased ROBOTS). Fetch `/robots.txt` (same idea as multiTabs → Directories in robots.txt). **txt** = raw `robots.txt` body; **web** = open Disallow directories in Firefox via `discover-robots:` → `misc/open-robots-tabs.sh` (only when `disallow_count` > 0). Skip `Disallow: /` (site root is not a directory to open). Keep query-string Disallow paths (Drupal `/?q=admin/` is not site root). Run does not open Firefox. Same **web** label on Audit.
- **ffuf / feroxbuster web:** green **web** (Firefox tabs) only when the run has at least one finding URL. Auto-filter-only / empty JSON → **txt** only. Count stored as `meta.url_count`. Same **web** label on Audit log and Target scans.
...
- **Active public-DNS check:** before httpx, `dig +timeout=2 +tries=1 +short A <host> @1.1.1.1` (parallel). RFC1918, `127.0.0.0/8`, `169.254.0.0/16`, or `0.0.0.0` drops that host from httpx and records `host<TAB>ip` in `tools/dns-private.tsv`. `tools/private-subs` is stored-private rows plus those leaks (dig IP wins). Do not rewrite the public IP in `tools/subdomains`. Do not use the operator resolver (VPN split-horizon is not a finding).
