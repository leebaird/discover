...
- **robots** / **nikto** / **feroxbuster** / **ffuf** always on expand when the UI is shown. **nmap** after robots when that IP has Shodan ports (`nmap -Pn -n --open -sTV -p <Shodan ports> <host>`; no HTTP pre-check). Order: **robots** → nmap (if Shodan ports) → nuclei (if product) → CMS tools → nikto → feroxbuster → ffuf.
