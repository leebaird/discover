    ...
    if [ -f "$src/css/modern.css" ]; then
        ...
        if [ -f "$report/pages/subdomains.htm" ]; then
            sed -i \
                ...
                -e 's|inc-host-scan\.js?v=[0-9]*|inc-host-scan.js?v=37|g' \
        ...
        for page in passive.htm active.htm audit.htm; do
            if [ -f "$report/pages/$page" ]; then
                sed -i \
                    -e 's|modern\.css?v=[^"]*|modern.css?v=active-status-info1|g' \
                ...
                if [ "$page" = "audit.htm" ]; then
                    sed -i \
                        -e 's|modern\.css?v=[^"]*|modern.css?v=audit-ferox1|g' \
                        -e 's|inc-audit-import\.js?v=[0-9]*|inc-audit-import.js?v=11|g' \
                        -e 's|inc-audit-config\.js?v=[0-9]*|inc-audit-config.js?v=14|g' \
                    ...
                    if ! grep -q 'inc-audit-config.js' "$report/pages/$page" 2>/dev/null; then
                        sed -i 's|</body>|<script src="../assets/javascript/inc-audit-config.js?v=14"></script>\n</body>|' \
                            "$report/pages/$page" 2>/dev/null || true
                    fi
...
need = [
    ("inc-subdomains-filter.js", '<script src="../assets/javascript/inc-subdomains-filter.js?v=17"></script>'),
    ...
    ("inc-host-scan.js", '<script src="../assets/javascript/inc-host-scan.js?v=37"></script>'),
]
