...
def scan(name, location, exclude_file, maxrtt, vdetection=False, full_scan=False, delay=0, run_msf=False):
    ...
    cmd = [
        ...
        f'-{scan_type}', f'-{udp_type}',
        f'-p', f'T:{tcp_ports},U:{udp_ports}',
        '-O', '--osscan-guess', '--max-os-tries', '1',
        '--max-retries', '2', '--min-rtt-timeout', '100ms',
        '--max-rtt-timeout', maxrtt, '--initial-rtt-timeout', '500ms',
        '--defeat-rst-ratelimit', '--min-rate', '450',
        '--max-rate', '15000', '--open', '--stats-every', '30s',
        '--scan-delay', str(delay), '-oA', f'{name}/nmap'
    ]
    ...
    with open(f'{name}/nmap.nmap', 'r') as f:
...
def process_nmap_output(name):
    ...
    for line in content.splitlines():
        if not re.search(r'(0000:|0010:|0020:|0030:|0040:|0050:|0060:|0070:|0080:|0090:|00a0:|00b0:|00c0:|00d0:|1 hop|closed|guesses|guessing|filtered|fingerprint|general purpose|initiated|latency|network distance|no exact os|no os matches|os cpe|please report|rttvar|scanned in|unreachable|warning)', line, re.IGNORECASE):
    ...
    # Extract open ports
    with open(f'{name}/nmap.txt', 'r') as f:
        content = f.read()
    ...
    with open(f'{name}/nmap.gnmap', 'r') as f:
...
def ports(name):
    ...
    udp_ports = [
        53, 67, 123, 137, 161, 407, 500, 523, 623, 1434, 1604, 1900, 2302, 2362, 3478, 3671, 4800, 5353, 5683, 6481, 17185, 31337, 44818, 47808
    ]
...
def generate_targets():
    ...
    if choice == "1":
        ...
        if ips:
            ...
            print(f"Results saved to ~/data/targets.txt")
        else:
        ...
        try:
            ...
            result = subprocess.run(['sudo', 'nmap', '-sn', network], 
                                   capture_output=True, text=True)
