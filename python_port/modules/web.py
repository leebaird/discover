    ...
    if choice == "1":
        ...
        try:
            result = subprocess.run(
                ['wget', '-q', f'{url}/robots.txt', '-O', 'robots.txt'],
                check=True
            )
...
def nikto_scan():
    ...
    if choice == "1":
        ...
        for ip in ips:
            ...
            cmd = [
                'nikto', '-h', ip, '-o', output_file, '-Format', 'htm'
            ]
        ...
        for target in targets:
            ...
            cmd = [
                'nikto', '-h', ip, '-p', port, '-o', output_file, '-Format', 'htm'
            ]
            process = subprocess.Popen(cmd)
...
def ssl_check():
    ...
    for target in targets:
        ...
        try:
            ...
            subprocess.run(
                [
                    'nmap', '-sV', '--script', 'ssl-enum-ciphers,ssl-cert,ssl-heartbleed,ssl-poodle,ssl-ccs-injection',
                    '-p', port, ip, '-oA', nmap_output
                ],
    ...
    with open(summary_file, 'w') as f:
        ...
        for target in targets:
            ...
            if os.path.exists(sslscan_output):
                with open(sslscan_output, 'r') as scan_file:
            ...
            if os.path.exists(nmap_output):
                with open(nmap_output, 'r') as scan_file:
                    content = scan_file.read()
