...
def update_system():
    ...
    try:
        result = subprocess.run(['sudo', 'nmap', '--script-updatedb'], 
...
def parse_xml():
    ...
    if choice == "1":  # Burp
        ...
        try:
            ...
            subprocess.run([
                "python3",
                str(parsers_dir / "parse-nexpose.py"),
                "--out", "nexpose.csv",
                location
            ], check=True)
        ...
        try:
            ...
            subprocess.run([
                "python3",
                str(parsers_dir / "parse-qualys.py"),
                "--out", "qualys.csv",
                location
            ], check=True)
...
def msf_web_api():
    ...
    try:
        result = subprocess.run(['service', 'postgresql', 'status'], 
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE, 
                               text=True)
