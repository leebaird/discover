...
def passive_recon():
    ...
    try:
        ...
        arin_xml = subprocess.run(
            ['curl', '-ks', f'https://whois.arin.net/rest/pocs;domain={domain}'], capture_output=True, text=True
        ).stdout
        ...
        if 'No Search Results' not in arin_xml:
            ...
            try:
                ...
                result = subprocess.run(['xmllint', '--format', 'tmp.xml'], capture_output=True, text=True)
            ...
            emails = []
            for url in urls:
                response = subprocess.run(['curl', '-k', '-s', url], capture_output=True, text=True)
                with open('tmp2.xml', 'w') as f:
                    f.write(response.stdout)

                try:
                    result = subprocess.run(['xml_grep', 'email', 'tmp2.xml', '--text_only'], capture_output=True, text=True)
    ...
    if os.path.exists('zhandles.txt'):
        names = []
        with open('zhandles.txt') as f:
            handles = f.read().splitlines()

        for handle in handles:
            response = subprocess.run(
                ['curl', '-ks', f'https://whois.arin.net/rest/poc/{handle}.txt'], capture_output=True, text=True
            )
    ...
    try:
        result = subprocess.run(['dnsrecon', '-d', domain, '-n', '8.8.8.8', '-t', 'std'], capture_output=True, text=True)

        # Process output
        records = []
        for line in result.stdout.splitlines():
            if not re.search(
                r'(all queries will|could not|dnskeys|dnssec|error|it is resolving|nsec3|performing|records|recursion|txt|version|wildcard resolution)',
                line,
                re.IGNORECASE,
            ):
    ...
    try:
        result = subprocess.run(['dnstwist', '--registered', domain], capture_output=True, text=True)
...
def find_registered_domains():
    ...
    for i, domain in enumerate(domains):
        ...
        try:
            whois_info = subprocess.run(['whois', '-H', domain], capture_output=True, text=True).stdout
