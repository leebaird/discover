...
cat tmp | egrep -v '(Could not|DNSSEC|Error|Performing|Records|Recursion|TXT|Version|\[)' | sed 's/\[\*\]//g; s/\[+\]//g; s/^[ \t]*//' | column -t | sort > records
...
cat z* | egrep -v '(@|:|\.|Atlanta|Boston|Detroit|Google|Maryland|North Carolina|Philadelphia|Planning|Search|United|University)' | sed 's/ And / and /; s/ Av / AV /g; 
