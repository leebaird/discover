...
declare -A WAF_SIGNATURES=(
    ...
    ["AWS WAF"]="aws-waf|awswaf|aws protect"
    ["Akamai"]="akamai|x-akamai-transformed"
    ["Imperva"]="imperva|incapsula"
    ["Sucuri"]="sucuri|cloudproxy"
    ["Fastly"]="fastly|x-fastly"
    ["F5 BIG-IP"]="big-ip|f5-trafficshield"
    ...
    ["USP Secure Entry"]="usp-secure-entry"
...
f_banner(){
 ...
 _    _    __    ____     ____  ____  ____  ____  ___  ____ 
( \/\/ )  /__\  ( ___)___(  _ \( ___)(_  _)( ___)/ __)(_  _)
 )    (  /(__)\  )__)(___))(_) ))__)   )(   )__)( (__   )(  
(__/\__)(__)(__)(__)     (____/(____) (__) (____)\___)  (__) 

${BLUE}WAF Detection Tool | ibrahimsql | $(date '+%Y-%m-%d %H:%M:%S')${NC}"
    echo
    echo
}
...
f_detect_waf() {
    ...
    for waf_name in "${!WAF_SIGNATURES[@]}"; do
        ...
        if grep -i -E "$signature" "$headers_file.combined" > /dev/null || grep -i -E "$signature" "$body_file" > /dev/null; then
            ...
            echo -e "[+] ${GREEN}Detected WAF: $waf_name${NC}"
        fi
    done
