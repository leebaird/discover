...
f_oauth_analyze() {
    ...
    echo -e "${BLUE}[*] Generating OAuth security report.${NC}"
    {
        ...
        if [ -f "$OUTPUT_DIR/oauth_test/oidc_config_formatted.json" ]; then
            ...
            jq -r 'keys[] as $k | "\($k): \(.[$k])"' "$OUTPUT_DIR/oauth_test/oidc_config_formatted.json" | grep -E "(issuer|authorization_endpoint|token_endpoint|userinfo_endpoint|jwks_uri|grant_types_supported|response_types_supported)"
        else
