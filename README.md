...
**Software fingerprint (expand):** `?software=` wins (e.g. Active Software versions link). Otherwise Discover reads the row Technologies tokens (keeps version when present, e.g. `Kibana:9.4.2`), then title, web server, and hostname labels (every label, e.g. `uatapi.iis.cgi.com` → IIS). Priority products include CMS, Kibana, Grafana, Elasticsearch, Jenkins, GitLab / Gitea / Gogs, Oracle (`oci-adb-control` / OCI ADB Control; not the bare `oci` label), Tomcat, IIS, nginx, Apache, PHP, Node.js. That product string is passed into `run-host-scan.sh` so nuclei Pass 1 uses product tags and Pass 2 can select CVE templates.
