...
On Active **Software versions**, versions that have NVD CVEs are linked to a filtered Subdomains view (`subdomains.htm?software=…`). **Alive by category** labels link to Subdomains filtered by that category (`subdomains.htm?category=Dev`, or `category=(none)` for empty). **Status codes** link to Subdomains with that HTTP status (`subdomains.htm?status=200`). **Top web servers** link by server name (`subdomains.htm?webserver=Apache`; parenthetical variants like `Apache (Debian)` match). The Active page header also has a centered **CVE search** bar: enter `CVE-YYYY-NNNNN` (or `YYYY-NNNNN`) to open Subdomains filtered to hosts running software linked to that CVE in the engagement NVD cache (`tools/cve-software-index.js`). Same filter banner / host-scan layout as software links (`subdomains.htm?cve=…`).
