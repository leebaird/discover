...
| **nmap** | TCP-only: `nmap -Pn -n --open -sTV -p <ports> <host>`. With UDP: `sudo nmap --privileged -Pn -n --open -sTV -sUV -p T:<tcp>,U:<udp> <host>` (Shodan transport, else Discover `UDP=` list; port 53 always TCP+UDP). UDP prompts for sudo in the host-scan terminal if needed. If raw UDP sendto is blocked (often VPN/TUN), TXT warns and does not keep `tcpwrapped` as a UDP fingerprint. **TXT**; **web** opens each http/https SERVICE in Firefox (`ssl/unknown` as https) | **Gated:** that IP has at least one port listed in the Shodan drop down. No HTTP pre-check |
